#!/usr/bin/env python3
"""
IntelliJ Single-Domain ML Training with Comprehensive Metrics
"""
...
def load_intellij_dataset():
    """Load IntelliJ behavioral dataset"""
    dataset = []
    with open('data/intellij_behavioral_dataset.csv', 'r') as f:
...
def print_classification_report(report, target_names=None):
    ...
    for cls, name in zip(classes, class_names):
        ...
        display_name = name[:28]  # Truncate long names
        print(f"{display_name:>30} {metrics['precision']:>9.2f} {metrics['recall']:>9.2f} {metrics['f1-score']:>9.2f} {metrics['support']:>9}")
    
    print()
    
    # Overall metrics
    print(f"{'accuracy':>30} {'':<9} {'':<9} {report['accuracy']:>9.2f} {report['macro_avg']['support']:>9}")
    print(f"{'macro avg':>30} {report['macro_avg']['precision']:>9.2f} {report['macro_avg']['recall']:>9.2f} {report['macro_avg']['f1-score']:>9.2f} {report['macro_avg']['support']:>9}")
    print(f"{'weighted avg':>30} {report['weighted_avg']['precision']:>9.2f} {report['weighted_avg']['recall']:>9.2f} {report['weighted_avg']['f1-score']:>9.2f} {report['weighted_avg']['support']:>9}")
...
def intellij_ml_training():
    """IntelliJ ML training with comprehensive evaluation"""
    
    print("🚀 INTELLIJ SINGLE-DOMAIN ML TRAINING")
    print("=" * 60)
    
    # Load dataset
    print("📊 Loading IntelliJ dataset...")
    dataset = load_intellij_dataset()
    ...
    print(f"\n📊 INTELLIJ RANDOM FOREST RESULTS:")
    print("=" * 60)
    ...
    # Save results
    with open('results/intellij_ml_test_results.csv', 'w', newline='') as f:
        fieldnames = [
    ...
    print(f"\n💾 Test results saved to: results/intellij_ml_test_results.csv")
    ...
    # Performance summary
    print(f"\n📊 INTELLIJ SINGLE-DOMAIN SUMMARY:")
    print(f"   Dataset: 125 refactorings, 24 types")
    print(f"   Test accuracy: {rf_report['accuracy']:.1%} ({len(correct_predictions)}/{len(test_set)})")
    print(f"   Macro F1-score: {rf_report['macro_avg']['f1-score']:.3f}")
    print(f"   Weighted F1-score: {rf_report['weighted_avg']['f1-score']:.3f}")
    
    return test_results, correct_predictions, rf_report

if __name__ == "__main__":
    intellij_ml_training()
