...
Generate results in same format as enhanced_generalization_results.md
"""
...
def generate_markdown_report(results):
    """Generate markdown report in same format as enhanced_generalization_results.md"""
...
"""
    ...
    # Add baseline first
    report += f"| **Training Baseline** (Commons/Gson) | {baseline['dataset_size']} | {baseline['accuracy']:.1%} | {baseline['complexity_range']} | {baseline['nesting_range']} | {baseline['dominant_refactoring']} ({baseline['dominant_percentage']:.1f}%) |\n"
    
    # Add other domains
    for domain_name, data in results.items():
        if domain_name != 'Commons/Gson':
...
def main():
    ...
    # Test model
    results, detailed_results = test_mixed_domain_model()
